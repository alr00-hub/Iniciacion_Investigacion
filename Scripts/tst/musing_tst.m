%% Script to assess quality of dsMUSING dataset

clear; clc;
n_songs = 12; % Number of unique songs
n_subs = 13;   % Number of subjects
...
for sub_idx = 13:n_subs
    ...
    for song_idx = 1:n_songs
        ...
        % Visualization
        if export
            create_figures(sub_idx, song_idx, EEG, data, n_channels, removed_percentage, corrs_all, ...
...
function export_title_page(sub_idx)
    ...
    axis off;
    text(0.5, 0.6, sprintf('dsMUSING (TFG)'), ...
...
function create_figures(sub_idx, song_idx, EEG, data, n_channels, removed_percentage, corrs_all, ...
    neighbor_matrix, c1_bad_channels, c2_bad_channels, c3_bad_channels, c4_bad_channels, ...
    bad_channels, low_band, high_band, fig_topos, fig_c1c2, fig_c3, fig_c4)
    ...
    if ~isempty(peripheral_c2_bad)
        ...
        for i = 1:n_bad
            ...
            title(['Channel ' EEG.chanlocs(ch).labels ' (number of neighbours: ' num2str(numel(neighbors)) ') Correlation: ' num2str(corrs_all(ch))]);
            xlabel('Time (ms)');
            ylabel('Channels');
        end
