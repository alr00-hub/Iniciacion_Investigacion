%% Script for dsARISTOTLE - Separate .set Files per Song
...
for sub_idx = 1:n_subs
    ...
    for song_idx = 1:n_songs
        ...
        song_folder = fullfile(subject_folder, ['ses-' num2str(song_idx, '%02d')]);
        ...
        EEG = pop_importdata('dataformat', 'matlab', 'nbchan', n_channels, 'data', EEG_song, ...
            'chanlocs', chanlocs, 'setname', ['sub-' subject_id 'ses-' num2str(song_idx)], ...
            'srate', Fs, 'pnts', size(EEG_song, 2), 'xmin', 0);
        
        % Add event marker with song rating
        EEG.event = struct();
        EEG.event(1).type = ['ses:' num2str(song_idx) '/rating:' num2str(song_ratings(song_idx))];
