...
function export_title_page(sub_idx)
    ...
    axis off;
    text(0.5, 0.6, sprintf('Quality Report\nSubject %03d', sub_idx), ...
        'FontSize', 24, 'HorizontalAlignment', 'center');
    text(0.5, 0.4, datestr(now, 'mmmm dd, yyyy'), ...
...
function create_figures(sub_idx, song_idx, EEG, data, n_channels, removed_percentage, ...
    ...
    for ch = c1_bad_channels
        plot(EEG.times, data(ch,:), 'b');

    end
    for ch = c2_bad_channels
        plot(EEG.times, data(ch,:), 'r');
    end
    ...
    n_bad = length(peripheral_c2_bad);
    if mod(n_bad, 2) ~= 0, n_bad = n_bad -1; end
    peripheral_c2_bad = peripheral_c2_bad(1:n_bad);
    if ~isempty(peripheral_c2_bad)
        fig_c2_neighbors = figure('Visible','off','Units','normalized','Position',[1 1 1 1]);
        set(fig_c2_neighbors, 'Visible', 'off');
        ...
        sgtitle(fig_c2_neighbors, ['C2 check:Time series of a bad channel (red) and its neighbours (blue) - Song ' num2str(song_idx)]);
        exportgraphics(fig_c2_neighbors, filename_pdf, 'Append', true, 'ContentType', 'image', 'Resolution', 300);
    end
